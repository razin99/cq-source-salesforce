...
func KrowProjectResourcesTable() *schema.Table {
	return &schema.Table{
		...
		Resolver: func(ctx context.Context, meta schema.ClientMeta, parent *schema.Resource, res chan<- any) error {
			...
			return queryGetAll(
				"select id,name,user_email__c,employment_end_date__c,employment_start_date__c,krow__team__c,krow__active__c,krow__location__c from krow__project_resources__c",
				cl.SalesForce,
				res,
			)
		},
